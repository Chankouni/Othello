 ...
 * location (the opposite players tokens are flipped). Othello makes use of the
 * OthelloBoard
 * 
 * @author arnold
 */
...
public class OthelloBoard {
	...
	private char alternation(int row, int col, int drow, int dcol) {
		...
		char cur_char = this.get(row, col); // either P1 or P2
		int next_row = row + drow;
		int next_col = col + dcol;
		if ((drow == 0 && dcol == 0) || (this.get(row, col) == EMPTY)) {
			return EMPTY;
		}
		while (this.validCoordinate(next_row, next_col)) {
			if (this.board[next_row][next_col] == otherPlayer(cur_char)) {
//reached other player
				return otherPlayer(cur_char); // returns person at end of line
			} else if (cur_char == this.board[next_row][next_col]) {
				next_row += drow;
				next_col += dcol;
				// continue if same player in row
				continue;
			} else if (this.get(next_row, next_col) == EMPTY) {
				return EMPTY;
			}
		}
		return EMPTY; // fail safe just in case somehow none of the conditions are met.
	}
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		int next_row = row + drow; // gets actual space for alternation
		int next_col = col + dcol; // gets actual space for alternation
		int counter = 0;

		if ((drow == 0 && dcol == 0) || this.get(next_row, next_col) == EMPTY) {
			return -1;
		} // if no direction or there does not exist an alternation

		while (this.validCoordinate(next_row, next_col)) {
			if (this.alternation(next_row, next_col, drow, dcol) == player) {
				this.board[next_row][next_col] = player;
	...
	/**
	 * Purpose of this method is to help greedy decide what to flip without flipping
	 * anything.
	 * 
	 * @param row
	 * @param col
	 ...
	 * @return
	 */
	protected int flipNum(int row, int col, int drow, int dcol, char player) {

		int next_row = row + drow; // gets actual space for alternation
		int next_col = col + dcol; // gets actual space for alternation
		int counter = 0;

		if ((drow == 0 && dcol == 0) || this.get(next_row, next_col) == EMPTY || this.get(row, col) != EMPTY) {
			return -1;
		} // if no direction or there does not exist an alternation
	...
	private char hasMove(int row, int col, int drow, int dcol) {
		...
		if ((drow == 0 && dcol == 0) || !this.validCoordinate(row, col) || this.get(row, col) != EMPTY) {
			return EMPTY;
		} // no direction to check, isn't a valid coordinate, or
// this place is already taken.
		else {
// must have alternation
			...
			for (int col = 0; col < this.dim; col++) {
				for (int drow = -1; drow < 2; drow++) {
					for (int dcol = -1; dcol < 2; dcol++) {
						if (this.hasMove(row, col, drow, dcol) == P1) {
							p1HasMove = true;
						}
						if (this.hasMove(row, col, drow, dcol) == P2) {
							p2HasMove = true;
						} else {
							continue;
						}
...
// job done!!
		...
		boolean moved = false;
		if (this.get(row, col) != EMPTY || (this.get(row, col) != EMPTY && !this.validCoordinate(row, col))) {

// if space is already taken or not valid coordinates
// we use and since getting something outside can return EMPTY
			return moved;
		}
		for (int drow = -1; drow < 2; drow++) {
			for (int dcol = -1; dcol < 2; dcol++) {
				if (this.flip(row, col, drow, dcol, player) != -1) {
// if is flippable, then convert space to player.
